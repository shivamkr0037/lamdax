...
from StarRobot import DRAGONS, LOGGER, dispatcher
from StarRobot.modules.connection import connected
from StarRobot.modules.disable import DisableAbleCommandHandler
from StarRobot.modules.helper_funcs.alternate import send_message, typing_action
from StarRobot.modules.helper_funcs.chat_status import user_admin
from StarRobot.modules.helper_funcs.extraction import extract_text
from StarRobot.modules.helper_funcs.filters import CustomFilters
from StarRobot.modules.helper_funcs.handlers import MessageHandlerChecker
from StarRobot.modules.helper_funcs.misc import build_keyboard_parser
from StarRobot.modules.helper_funcs.msg_types import get_filter_type
from StarRobot.modules.helper_funcs.string_handling import (
    button_markdown_parser,
    escape_invalid_curly_brackets,
    markdown_to_html,
    split_quotes,
)
from StarRobot.modules.sql import cust_filters_sql as sql
...
def list_handlers(update, context):
    ...
    if not all_handlers:
        send_message(
            update.effective_message, "No filters saved in {}❤️‍🔥".format(chat_name)
        )
        return
...
def filters(update, context):
    ...
    if not msg.reply_to_message and len(args) < 2:
        send_message(
            update.effective_message,
            "Please provide keyboard keyword for this filter to reply with❤️‍🔥",
        )
        return
    ...
    if msg.reply_to_message:
        if len(args) < 2:
            send_message(
                update.effective_message,
                "Please provide keyword for this filter to reply with❤️‍🔥",
            )
            return
        else:
            keyword = args[1]
    ...
    if not msg.reply_to_message and len(extracted) >= 2:
        ...
        if not text:
            send_message(
                update.effective_message,
                "There is no note message - You can't JUST have buttons, you need a message to go with it❤️‍🔥",
            )
            return
        ...
        send_message(
            update.effective_message,
            "Please provide keyword for this filter reply with❤️‍🔥",
        )
        return
        ...
        send_message(update.effective_message, "Invalid filter❤️‍🔥")
        return
...
def stop_filter(update, context):
    ...
    if len(args) < 2:
        send_message(update.effective_message, "What should i stop?❤️‍🔥")
        return

    chat_filters = sql.get_chat_triggers(chat_id)

    if not chat_filters:
        send_message(update.effective_message, "No filters active here❤️‍🔥")
        return
    ...
    send_message(
        update.effective_message,
        "That's not a filter - Click: /filters to get currently active filters.❤️‍🔥",
    )
...
def reply_filter(update, context):
    ...
    for keyword in chat_filters:
        ...
        if re.search(pattern, to_match, flags=re.IGNORECASE):
            ...
            if filt.reply == "there is should be a new reply❤️‍🔥":
                ...
                if filt.reply_text:
                    ...
                    if text.startswith("~!") and text.endswith("!~"):
                        ...
                        try:
                            ...
                            if (
                                excp.message
                                == "Wrong remote file identifier specified: wrong padding in the string❤️‍🔥"
                            ):
                                context.bot.send_message(
                                    chat.id,
                                    "Message couldn't be sent, Is the sticker id valid?❤️‍🔥",
                                )
                                return
                            else:
                ...
                if filt.file_type in (sql.Types.BUTTON_TEXT, sql.Types.TEXT):
                    ...
                    try:
                        ...
                        send_message(
                            message,
                            "I don't have the permission to send the content of the filter.❤️‍🔥",
                        )
                break
            else:
                if filt.is_sticker:
...
def rmall_filters(update, context):
    ...
    if member.status != "creator" and user.id not in DRAGONS:
        ...
        update.effective_message.reply_text(
            f"Are you sure you would like to stop ALL filters in {chat.title}? This action cannot be undone.❤️‍🔥",
...
def rmall_callback(update, context):
    ...
    if query.data == "filters_rmall":
        if member.status == "creator" or query.from_user.id in DRAGONS:
            ...
            if not allfilters:
                msg.edit_text("No filters in this chat, nothing to stop❤️‍🔥")
                return
        ...
        if member.status == "administrator":
            query.answer("Only owner of the chat can do this.❤️‍🔥")

        if member.status == "member":
            query.answer("You need to be admin to do this.❤️‍🔥")
    elif query.data == "filters_cancel":
        if member.status == "creator" or query.from_user.id in DRAGONS:
            msg.edit_text("Clearing of all filters has been cancelled.❤️‍🔥")
            return
        if member.status == "administrator":
            query.answer("Only owner of the chat can do this.❤️‍🔥")
        if member.status == "member":
...
def get_exception(excp, filt, chat):
    ...
    elif excp.message == "Reply message not found❤️‍🔥":
        return "noreply"
    else:
